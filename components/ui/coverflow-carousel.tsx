...
export interface CoverflowSlide {
  /** Image source — only rendered when `emoji` is absent. */
  src?: string;
  alt: string;
  /** When set, renders an emoji tile on the slide's `gradient` instead of an image. */
  emoji?: string;
  gradient?: string;
  /** Shows a pulsing LIVE badge overlaid on the card. */
  live?: boolean;
  /** Small date · time pill overlaid bottom-left, like a muhurat stamp. */
  dateLabel?: string;
  title?: string;
  subtitle?: string;
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-xl will-change-transform transition-[box-shadow] duration-300",
                  // A soft glow ring singles out the active pooja in the rake.
                  index === selected &&
                    "ring-2 ring-saffron-400/80 shadow-[0_0_24px_rgba(249,125,20,0.4)]",
                  cardClassName,
                )}
              ...
              >
                {slide.emoji ? (
                  <div
                    className={`flex h-full w-full select-none items-center justify-center bg-gradient-to-br ${slide.gradient ?? "from-saffron-400 to-maroon-600"}`}
                  >
                    {/* The emoji lifts gently only while hovered, and settles
                        back when the cursor leaves — no looping motion. */}
                    <span className="text-5xl drop-shadow-lg transition-transform duration-300 ease-out hover:-translate-y-1.5 hover:scale-110">
                      {slide.emoji}
                    </span>
                  </div>
                ) : (
                  ...
                  <img
                    ...
                    draggable={false}
                    onError={(event) => {
                      // Fall back to the muted tile if the image ever fails.
                ...
                {slide.live && (
                  <span
                    className={cn(
                      "absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-maroon-700 shadow transition-all duration-300",
                      // The centre card's badge scales up with a red glow so the
                      // active pooja stands out from the rake.
                      index === selected
                        ? "scale-110 shadow-[0_0_18px_rgba(239,68,68,0.6)]"
                        : "scale-100",
                    )}
                  >
                ...
                {slide.dateLabel && (
                  <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1.5 whitespace-nowrap rounded-full bg-black/40 px-2.5 py-1 text-[10px] font-semibold text-white backdrop-blur-sm">
